...
function Base.isless(o1::Op, o2::Op)
...
function Base.isless(o1::Prod{Op}, o2::Prod{Op})
...
function Base.isless(o1::Scaled{C1,Prod{Op}}, o2::Scaled{C2,Prod{Op}}) where {C1,C2}
...
function Base.convert(::Type{Scaled{C1,Prod{Op}}}, o::Scaled{C2,Prod{Op}}) where {C1,C2}
...
Base.:+(o1::Op, o2::Op) = Applied(sum, ([o1, o2],))
Base.:*(o1::Op, o2::Op) = Applied(prod, ([o1, o2],))
Base.:-(o::Op) = -one(Int) * o
Base.:-(o1::Op, o2::Op) = o1 + (-o2)

Base.:*(c::Number, o::Op) = Applied(*, (c, o))
Base.:*(o::Op, c::Number) = Applied(*, (c, o))
Base.:/(o::Op, c::Number) = Applied(*, (inv(c), o))

Base.:*(c::Number, o::Prod{Op}) = Applied(*, (c, o))
Base.:*(o::Prod{Op}, c::Number) = Applied(*, (c, o))
Base.:/(o::Prod{Op}, c::Number) = Applied(*, (inv(c), o))

# 1.3 * Op("X", 1) + Op("X", 2)
# 1.3 * Op("X", 1) * Op("X", 2) + Op("X", 3)
Base.:+(co1::Scaled{C}, o2::Op) where {C} = co1 + one(C) * o2

# Op("X", 1) + 1.3 * Op("X", 2)
Base.:+(o1::Op, co2::Scaled{C}) where {C} = one(C) * o1 + co2

Base.:*(o1::Op, o2::Sum) = Applied(sum, (map(a -> o1 * a, only(o2.args)),))
Base.:*(o1::Sum, o2::Op) = Applied(sum, (map(a -> a * o2, only(o1.args)),))

# 1.3 * Op("X", 1) + Op("X", 2) * Op("X", 3)
# 1.3 * Op("X", 1) * Op("X", 2) + Op("X", 3) * Op("X", 4)
Base.:+(co1::Scaled{C}, o2::Prod{Op}) where {C} = co1 + one(C) * o2

# 1.3 * Op("X", 1) * Op("X", 2)
Base.:*(co1::Scaled{C}, o2::Op) where {C} = co1 * (one(C) * o2)

Base.exp(o::Op) = Applied(exp, (o,))

Base.adjoint(o::Op) = Applied(adjoint, (o,))
Base.adjoint(o::LazyApply.Adjoint{Op}) = only(o.args)

Base.:*(o1::Exp{Op}, o2::Op) = Applied(prod, ([o1, o2],))
...
function Base.:+(o1::OpSumLike, o2::Tuple)
  return o1 + op_term(o2)
end

function Base.:+(o1::Tuple, o2::OpSumLike)
...
function Base.show(io::IO, ::MIME"text/plain", o::Op)
  print(io, which_op(o))
  print(io, sites(o))
  if !isempty(params(o))
    print(io, params(o))
...
Base.show(io::IO, o::Op) = show(io, MIME("text/plain"), o)

function Base.show(io::IO, ::MIME"text/plain", o::Prod{Op})
  for n in 1:length(o)
    print(io, o[n])
    if n < length(o)
      print(io, " ")
...
Base.show(io::IO, o::Scaled{C,Prod{Op}}) where {C} = show(io, MIME("text/plain"), o)

function Base.show(io::IO, ::MIME"text/plain", o::LazyApply.Adjoint{Op})
  print(io, o')
  print(io, "'")
  return nothing
end
Base.show(io::IO, o::LazyApply.Adjoint{Op}) = show(io, MIME("text/plain"), o)
