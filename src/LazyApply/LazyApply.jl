...
# Fix ambiguity error with `Applied * Sum`.
# TODO: Share code, define `mul_sum`.
(a1::Exp * a2::Sum) = Applied(sum, (map(a -> a1 * a, only(a2.args)),))
# Fix ambiguity error with `Sum * Applied`.
# TODO: Share code, define `mul_sum`.
(a1::Sum * a2::Exp) = Applied(sum, (map(a -> a * a2, only(a1.args)),))
